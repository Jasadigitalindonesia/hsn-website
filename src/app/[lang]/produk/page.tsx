        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-1/4 flex-shrink-0">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
              ...
              <ul className="space-y-2">
                <li>
                  <Link href={`/${lang}/produk`} className="flex items-center justify-between p-3 rounded-lg bg-primary/5 text-primary font-bold transition-colors">
                ...
                {categories.map((cat: string, i: number) => (
                  <li key={i}>
                    <Link href={`/${lang}/kategori/${cat.toLowerCase().replace(/\s+/g, '-')}`} className="flex items-center justify-between p-3 rounded-lg text-gray-600 hover:bg-gray-50 hover:text-primary transition-colors font-medium group">
                      <span>{cat}</span>
          ...
          <div className="w-full lg:w-3/4">
            ...
            <div className="flex justify-between items-center mb-6">
              <p className="text-gray-500 text-sm font-medium">Menampilkan 1–9 dari 120 hasil</p>
                  ...
                  <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col h-full">
                    <div className="relative overflow-hidden aspect-[4/3] bg-gray-50">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <Link href={`/${lang}/produk/detail`} className="bg-white text-primary hover:bg-primary-hover hover:text-white font-bold py-2 px-6 rounded-full transition-colors transform translate-y-4 group-hover:translate-y-0 duration-300">
                          Detail Produk
                        </Link>
                      </div>
                    </div>
                    <div className="p-5 flex-grow flex flex-col">
                      ...
                      <h3 className="font-bold text-gray-900 text-lg mb-4 flex-grow leading-snug hover:text-primary transition-colors">
                        <Link href={`/${lang}/produk/detail`}>{prod.name}</Link>
                      </h3>
                    </div>
                  </div>
                </AnimatedSection>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center mt-12">
                <nav className="flex items-center gap-2">
                  <Link 
                    href={`/${lang}/produk?page=${Math.max(1, currentPage - 1)}`} 
                  ...
                  <Link 
                    href={`/${lang}/produk?page=${Math.min(totalPages, currentPage + 1)}`} 
