          ...
          <div className="w-full lg:w-2/3 xl:w-3/4">
                ...
                <article key={article.id || i} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group flex flex-col h-full">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <h3 className="font-bold text-gray-900 text-xl mb-3 line-clamp-2 group-hover:text-primary transition-colors">
                      <Link href={`/${lang}/artikel/detail`}>{article.title}</Link>
                    </h3>
                    <p className="text-gray-600 mb-6 flex-grow line-clamp-3 leading-relaxed">
                      {article.desc}
                    </p>
                    <Link href={`/${lang}/artikel/detail`} className="inline-flex items-center gap-2 text-primary font-bold hover:text-primary-hover transition-colors mt-auto group/btn">
            ...
            <div className="flex justify-center mt-12">
              <nav className="flex items-center gap-2">
                <Link href={`/${lang}/artikel`} className="w-10 h-10 rounded-lg border border-gray-200 text-gray-500 hover:border-primary hover:text-primary flex items-center justify-center transition-colors">
                  &lt;
                </Link>
                <Link href={`/${lang}/artikel`} className="w-10 h-10 rounded-lg bg-primary text-white font-bold flex items-center justify-center shadow-md">
                  1
                </Link>
                <Link href={`/${lang}/artikel`} className="w-10 h-10 rounded-lg border border-gray-200 text-gray-700 hover:border-primary hover:text-primary font-medium flex items-center justify-center transition-colors">
                  2
                </Link>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <ul className="space-y-2">
                {categories.map((cat, i) => (
                  <li key={i}>
                    <Link href={`/${lang}/artikel/kategori`} className="flex items-center justify-between p-2 rounded-lg text-gray-600 hover:bg-primary/5 hover:text-primary transition-colors font-medium group">
              ...
              <div className="space-y-4">
                {recentPosts.map((post: any, i: number) => (
                  <div key={i} className="flex gap-4 group">
                    <Link href={`/${lang}/artikel/detail`} className="shrink-0">
                      <div className="w-20 h-20 rounded-lg overflow-hidden">
                        <img src={post.img} alt={post.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                      </div>
                    </Link>
                    <div className="flex flex-col justify-center">
                      <Link href={`/${lang}/artikel/detail`} className="font-bold text-gray-900 text-sm mb-1 line-clamp-2 group-hover:text-primary transition-colors leading-tight">
                        {post.title}
                      </Link>
